...
import Hoje       from './Hoje.jsximport React, { useState, useEffect } from 'react';
import { useLocalStorage } from './useLocalStorage.js';
import { EVENTOS_INICIAIS } from './dados.js';
import { hojeISO } from './plano.js';
...
export default function App() {
  ...
  const [config, setConfig] = useLocalStorage('ps_config', {
    ...
    cc50Modulo: 'Módulo 1 – C',
    cargaHoraria: 2,
    fontSize: 16,
  });
  const [zenConteudo, setZenConteudo] = useState(null);
  const [notificacao, setNotificacao] = useState(null);
  const [dataSelecionada, setDataSelecionada] = useState(hojeISO());
  ...
  function abrirZen(conteudo) {
    setZenConteudo(conteudo);
  }

  if (zenConteudo) {
    return <ZenMode conteudo={zenConteudo} onSair={() => setZenConteudo(null)} />;
  }

  const props = {
    eventos, concluirEvento, reabrirEvento,
    aulasAssistidas, marcarAulaAssistida,
    estudosConcluidos, marcarEstudoConcluido,
    edicoesManha, editarManha,
    config, setConfig,
